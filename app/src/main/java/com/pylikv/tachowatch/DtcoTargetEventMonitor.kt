...
class DtcoTargetEventMonitor(private val context: Context, private val listener: Listener? = null) {
    ...
    companion object {
        private const val VERSION = "DTCO-LIVE-DID-v9.5"
        private const val NEXT_CYCLE_MS = 15000L
        private const val RESPONSE_TIMEOUT_MS = 3500L
        private const val MAX_VISIBLE_LOG_LINES = 500
        private const val UI_LOG_THROTTLE_MS = 750L
    ...
    private val workerThread = HandlerThread("dtco-target-monitor").apply { start() }
    private val handler = Handler(workerThread.looper)
    private val uiHandler = Handler(Looper.getMainLooper())
    private val logDispatchScheduled = AtomicBoolean(false)
    ...
    fun connect(d: BluetoothDevice, newSession: Boolean = true) {
        if (!hasConnectPermission()) {
            log("ERROR: no BLUETOOTH_CONNECT")
            return
        }
        closeGatt()
        device = d
        ...
        if (newSession || logFile == null) {
            synchronized(lineLock) { lines.clear() }
            previous.clear()
            startLogFile()
            log("============================================================")
            log("DTCO LIVE DID MONITOR v9.5")
            log("READ ONLY: UDS 0x22 only")
            log("AUTOSAVE: every log line is flushed immediately to ${getCurrentLogFileName() ?: "internal file"}")
            log("VISIBLE LOG: last $MAX_VISIBLE_LOG_LINES lines only; full history remains in file")
            ...
            log("========== RECONNECT: continuing same autosave session ==========")
        }
        ...
        try {
            ...
            listener?.onConnectionStateChanged(false, safeName(d))
        }
    }
    ...
    private fun startLogFile() {
        synchronized(fileLock) {
            ...
            val file = File(dir, "DTCO_LIVE_DID_v9_5_$stamp.txt")
            try {
                if (!file.exists()) file.createNewFile()
                logFile = file
                logWriter = BufferedWriter(OutputStreamWriter(FileOutputStream(file, true), Charsets.UTF_8), 16 * 1024)
            } catch (_: Throwable) {
                logFile = null
                logWriter = null
            }
    ...
    private fun closeWriterLocked() {
        ...
        try { logWriter?.close() } catch (_: Throwable) {}
        logWriter = null
    }
        ...
        return try {
            synchronized(fileLock) {
                ...
                if (f != null && f.exists()) {
                    ...
                    out.write(getLog().toByteArray(Charsets.UTF_8))
                }
            }
            out.flush()
            true
    ...
    private fun subscribe(g: BluetoothGatt, uuid: UUID) {
        val c = g.getService(SERVICE)?.getCharacteristic(uuid) ?: run {
            log("CHAR $uuid NOT FOUND")
            return
        }
        val ok = g.setCharacteristicNotification(c, true)
        log("SUBSCRIBE $uuid local=$ok")
        val d = c.getDescriptor(CCCD) ?: run {
            log("CCCD NOT FOUND")
            return
        }
    ...
    private fun startHandshake(g: BluetoothGatt) {
        if (!fifoSubscribed || !creditsSubscribed) {
            log("DIAG subscriptions incomplete")
            return
        }
        grantRx(g, 1)
    ...
    private fun incoming(g: BluetoothGatt, c: BluetoothGattCharacteristic, v: ByteArray) {
        if (c.uuid == CREDITS) {
            log("RX CREDITS ${hex(v)} (informational)")
            return
        }
    ...
    private fun sendOpen(g: BluetoothGatt) {
        if (openSent) return
        openSent = true
        writeFifo(g, byteArrayOf(1, 1, 0x31, 0x01, 0xF2.toByte(), 0x11), "OPEN RHMI")
    }

    private fun sendStatus(g: BluetoothGatt) {
        if (statusSent) return
        statusSent = true
        writeFifo(g, byteArrayOf(1, 1, 0x31, 0x03, 0xF2.toByte(), 0x11), "RHMI STATUS")
    }

    private fun startCycle(g: BluetoothGatt) {
        if (!connected || !rhmiOpen || g !== gatt) return
        cycle++
        index = 0
        waitingDid = null
        log("---------------- CYCLE $cycle ----------------")
    ...
    private fun sendNext(g: BluetoothGatt) {
        ...
        if (index >= dids.size) {
            ...
            handler.postDelayed({ if (connected && rhmiOpen && g === gatt) startCycle(g) }, NEXT_CYCLE_MS)
            return
        }
        val id = dids[index]
        waitingDid = id
    ...
    private fun onPositive(g: BluetoothGatt, id: Int, data: ByteArray) {
        if (g !== gatt) return
        if (waitingDid != id) {
            log("UNEXPECTED ${did(id)}=${hex(data)}")
            return
        }
        timeoutToken++
        val old = previous[id]
        ...
        if (old == null) {
            log("BASELINE ${did(id)} ${names[id]} = ${hex(data)}")
        } else if (changed) {
            log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        ...
        listener?.onDidUpdate(id, hex(data), decoded.ifBlank { "Пока не расшифровано" }, changed, diff, ts)
    ...
    fun addMarker(text: String) {
        if (text.isNotBlank()) log("========== USER MARKER: ${text.trim()} ==========")
    }

    fun clearLog() {
        synchronized(lineLock) { lines.clear() }
        log("Visible log cleared; autosaved file preserved")
    }

    fun getLog(): String = synchronized(lineLock) { lines.joinToString("\n") }
    ...
    fun shutdown() {
        disconnect()
        synchronized(fileLock) { closeWriterLocked() }
    ...
    private fun log(s: String) {
        ...
        synchronized(lineLock) {
            lines.addLast(line)
            while (lines.size > MAX_VISIBLE_LOG_LINES) lines.removeFirst()
        }
        synchronized(fileLock) {
            try {
        ...
        if (logDispatchScheduled.compareAndSet(false, true)) {
            uiHandler.postDelayed({
                logDispatchScheduled.set(false)
