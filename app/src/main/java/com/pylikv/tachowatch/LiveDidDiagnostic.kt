...
class LiveDidDiagnostic(private val context: Context, private val listener: Listener) {
    ...
    fun connect(device: BluetoothDevice) {
        ...
        disconnect(false)
        lines.clear(); txCredits = 0; openSent = false; statusSent = false; rhmi = false; index = 0; waiting = false; token++
        log("LIVE START")
        gatt = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) device.connectGatt(context, false, cb, BluetoothDevice.TRANSPORT_LE)
    ...
    private fun incoming(g: BluetoothGatt, c: BluetoothGattCharacteristic, v: ByteArray) {
        ...
        if (a.size >= 3 && u(a[0]) == 0x62) {
            ...
            if (waiting && index < dids.size && did == dids[index]) { waiting = false; token++; index++; handler.postDelayed({ requestNext(g) }, 140) }
            return
        }
    ...
    private fun requestNext(g: BluetoothGatt) {
        ...
        if (index >= dids.size) { log("LIVE COMPLETE"); return }
        grant(g)
        handler.postDelayed({
    ...
    private fun writeNoResponse(g: BluetoothGatt, c: BluetoothGattCharacteristic, v: ByteArray): Boolean = try {
        if (Build.VERSION.SDK_INT >= 33) g.writeCharacteristic(c, v, BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE) == BluetoothGatt.GATT_SUCCESS
    ...
    private fun decode(did: Int, b: ByteArray): String = when (did) {
        0xF903 -> if (b.isEmpty()) "—" else when (u(b[0]) and 7) { 0 -> "ОТДЫХ / ПЕРЕРЫВ"; 1 -> "ГОТОВНОСТЬ"; 2 -> "ДРУГАЯ РАБОТА"; 3 -> "ВОЖДЕНИЕ"; else -> "КОД ${u(b[0]) and 7}" }
        0xF923,0xF925,0xF927,0xF938 -> if (b.size < 2) "—" else { val m=(u(b[0]) shl 8) or u(b[1]); "$m мин = ${m/60}:${String.format("%02d",m%60)}" }
        0xF931 -> if (b.size >= 72) "${clean(b.copyOfRange(0,36))} ${clean(b.copyOfRange(36,72))}".trim() else clean(b)
        else -> hex(b)
    }
    private fun clean(b: ByteArray) = buildString { b.forEach { val x=u(it); if (x in 32..126) append(x.toChar()) else if (x==0) append(' ') } }.trim()
    private fun log(s: String) { lines.add(s); while (lines.size > 200) lines.removeAt(0); listener.onLiveLog(lines.joinToString("\n")) }
