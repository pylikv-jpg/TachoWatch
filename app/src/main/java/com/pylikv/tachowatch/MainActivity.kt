...
class MainActivity : AppCompatActivity(), DtcoBluetoothDiagnostic.Listener {
    ...
    companion object {
        ...
        private const val RESULT_MARKER = "===== TEST-9 CARD DATA SEARCH RESULT ====="
    }
    ...
    private fun createInterface() {
        ...
        val subtitle = TextView(this).apply {
            text = "DTCO Bluetooth diagnostics — TEST-9A CARD DATA"
            textSize = 12f
        ...
        deviceCard.addView(devicesScroll, LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, dp(130)))
        ...
        controlHeader.addView(label("ДИАГНОСТИКА BLE / GATT"), LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f))
        ...
        connectButton = button("Подключиться и начать TEST-9A", COLOR_BLUE).apply {
            isEnabled = false
            alpha = 0.55f
            setOnClickListener {
        ...
        manualGattButton = button("Проверить GATT", COLOR_CYAN).apply {
            isEnabled = false
            alpha = 0.55f
            setOnClickListener { diagnostic.manualGattCheck() }
        }
        ...
        logHeader.addView(label("ДИАГНОСТИЧЕСКИЙ ЖУРНАЛ"), LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f))
        ...
        copyResultButton = button("Копировать результат", COLOR_GREEN).apply {
            setOnClickListener { copyResultOnly() }
        }
        copyAllButton = button("Копировать весь журнал", COLOR_CYAN).apply {
            setOnClickListener { copyText(renderedLog, "Весь журнал скопирован") }
        }
        ...
        logText = TextView(this).apply {
            text = "Ожидание запуска диагностики..."
            textSize = 11.5f
            setTextColor(COLOR_TEXT)
    ...
    private fun copyResultOnly() {
        val full = renderedLog
        val idx = full.lastIndexOf(RESULT_MARKER)
        if (idx < 0) {
            Toast.makeText(this, "Итог TEST-9 ещё не сформирован", Toast.LENGTH_SHORT).show()
            return
        }
        val start = full.lastIndexOf('\n', idx).let { if (it >= 0) it + 1 else idx }
        copyText(full.substring(start).trim(), "Результат TEST-9 скопирован")
    }
    ...
    private fun copyText(text: String, message: String) {
        ...
        clipboard.setPrimaryClip(ClipData.newPlainText("TachoWatch TEST-9A", text))
    ...
    private fun loadBondedDevices() {
        ...
        if (list.isEmpty()) {
            devicesContainer.addView(TextView(this).apply {
                text = "Сопряжённых Bluetooth-устройств нет"
                setTextColor(COLOR_MUTED)
                textSize = 12f
            })
            return
        ...
        list.forEach { d ->
            ...
            devicesContainer.addView(b)
            devicesContainer.addView(space(4))
        }
    }
    ...
    override fun onConnectionStateChanged(connected: Boolean, deviceName: String?) {
        runOnUiThread {
            if (connected) {
                ...
                manualGattButton.isEnabled = true
                manualGattButton.alpha = 1f
            } else {
                setStatus("Отключено", COLOR_ORANGE)
                manualGattButton.isEnabled = false
                manualGattButton.alpha = 0.55f
            }
        }
    }
    ...
    private fun card() = LinearLayout(this).apply {
        ...
        background = rounded(COLOR_CARD, dp(12).toFloat(), COLOR_BORDER)
    }
    ...
    private fun button(textValue: String, bg: Int) = Button(this).apply {
        ...
        textSize = 12f
        setTextColor(COLOR_TEXT)
        background = rounded(bg, dp(10).toFloat(), COLOR_BORDER)
        minHeight = dp(44)
    ...
    private fun smallButton(textValue: String) = Button(this).apply {
        ...
        textSize = 11f
        setTextColor(COLOR_TEXT)
        background = rounded(COLOR_CARD_ALT, dp(8).toFloat(), COLOR_BORDER)
        minHeight = dp(36)
