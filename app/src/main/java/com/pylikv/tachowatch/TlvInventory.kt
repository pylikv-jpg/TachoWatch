...
object TlvInventory {
    ...
    private fun renderDriverActivity(result:Result):String{
        ...
        entries.forEach{entry->
            ...
            val shown=records.size;sb.appendLine("DecodedDailyRecords=${records.size}");sb.appendLine("ShowingAll=$shown");sb.appendLine()
    ...
    private fun decodeCircularRecords(buffer:ByteArray,oldest:Int,newest:Int):List<DailyRecord>{val out=ArrayList<DailyRecord>();var offset=oldest;val visited=HashSet<Int>();while(true){if(offset !in buffer.indices||!visited.add(offset))return out;val header=readCyclic(buffer,offset,12)?:return out;val prev=be16(header,0);val len=be16(header,2);if(len<12||len>buffer.size||len%2!=0)return out;val raw=readCyclic(buffer,offset,len)?:return out;val date=be32(raw,4);if(date==0L)return out;val changes=ArrayList<ActivityChange>();var p=12;var last=-1;while(p+1<raw.size){val v=be16(raw,p);val minute=v and 0x07FF;if(minute !in 0..1439||last>minute)break;changes+=ActivityChange(v,minute,(v ushr 11) and 3,if(((v ushr 15) and 1)==0)1 else 2,((v ushr 14) and 1)!=0,((v ushr 13) and 1)==0);last=minute;p+=2};out+=DailyRecord(offset,prev,len,date,be16(raw,8),be16(raw,10),changes);if(offset==newest)return out;offset=(offset+len)%buffer.size}}
