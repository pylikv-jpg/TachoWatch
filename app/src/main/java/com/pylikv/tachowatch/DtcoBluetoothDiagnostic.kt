...
class DtcoBluetoothDiagnostic(private val context: Context, private val listener: Listener? = null) {
    ...
    fun manualGattCheck(){log("===== MANUAL STATUS =====");log("connected=$connected RHMI=$rhmi FIFO=$fifoSub Credits=$creditSub TX=$credits reconnectUsed=$reconnectUsed");log("scan=$index/${dids.size} scanning=$scanning waiting=$waiting DID=${currentDid?.let(::hd)?:"NONE"}")}
    ...
    private val cb=object:BluetoothGattCallback(){
        ...
        override fun onDescriptorWrite(g:BluetoothGatt,d:BluetoothGattDescriptor,status:Int){val u=d.characteristic.uuid;log("CCCD $u status=$status");if(u==FIFO){fifoSub=status==BluetoothGatt.GATT_SUCCESS;handler.postDelayed({if(connected)subscribe(g,CREDITS)},150)}else if(u==CREDITS){creditSub=status==BluetoothGatt.GATT_SUCCESS;if(creditSub){log("CREDITS subscribed; granting initial RX credit with NO_RESPONSE");handler.postDelayed({if(connected&&credits==0&&!openSent)grant(g,"INITIAL")},300)}else stop("CREDITS CCCD subscription failed")}}
        override fun onCharacteristicWrite(g:BluetoothGatt,c:BluetoothGattCharacteristic,status:Int){log("CHAR-WRITE uuid=${c.uuid} status=$status type=${writeTypeName(c.writeType)}")}
    ...
    private fun logChar(label:String,c:BluetoothGattCharacteristic?){if(c==null){log("$label characteristic NOT FOUND");return};log("$label props=0x${Integer.toHexString(c.properties)} perms=0x${Integer.toHexString(c.permissions)} writeType=${writeTypeName(c.writeType)}");log("$label flags: WRITE=${hasProp(c,BluetoothGattCharacteristic.PROPERTY_WRITE)} WRITE_NR=${hasProp(c,BluetoothGattCharacteristic.PROPERTY_WRITE_NO_RESPONSE)} INDICATE=${hasProp(c,BluetoothGattCharacteristic.PROPERTY_INDICATE)}")}
    private fun hasProp(c:BluetoothGattCharacteristic,p:Int)=(c.properties and p)!=0

    @SuppressLint("MissingPermission") private fun subscribe(g:BluetoothGatt,uuid:UUID){val c=g.getService(SERVICE)?.getCharacteristic(uuid)?:return;val n=g.setCharacteristicNotification(c,true);log("${if(uuid==FIFO)"FIFO" else "CREDITS"} setCharacteristicNotification=$n");c.getDescriptor(CCCD)?.let{log("${if(uuid==FIFO)"FIFO" else "CREDITS"} CCCD=${writeDesc(g,it,BluetoothGattDescriptor.ENABLE_INDICATION_VALUE)}")}}
