...
class DtcoBluetoothDiagnostic(private val context: Context, private val listener: Listener? = null) {
    ...
    private fun reset() {
        ...
        currentDid=null; finished=false; reqGen++; creditGen++
    }

    fun manualGattCheck() {
        log("===== MANUAL STATUS =====")
        log("connected=$connected RHMI=$rhmi FIFO=$fifoSub Credits=$creditSub TX=$credits")
    ...
    private val cb = object : BluetoothGattCallback() {
        ...
        override fun onConnectionStateChange(g: BluetoothGatt, status: Int, newState: Int) {
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                connected=false; waiting=false; reqGen++; creditGen++
                notifyConnection(false, safeName(g.device)); log("BLE DISCONNECTED")
            }
        }
    ...
    private fun incoming(g:BluetoothGatt,c:BluetoothGattCharacteristic,v:ByteArray) {
        if (c.uuid==CREDITS) {
            ...
            credits+=n; log("TX-CREDIT RX +$n => $credits"); creditGen++; pump(g); return
        }
        ...
        if(a.size>=4 && u(a[0])==0x71 && u(a[1])==1 && u(a[2])==0xF2 && u(a[3])==0x11) {
            log("OPEN RHMI POSITIVE"); grant(g,"RHMI-STATUS"); handler.postDelayed({sendStatus(g)},400); return
        }
        ...
        if(a.size>=3 && u(a[0])==0x7F && u(a[1])==0x22 && waiting && currentDid!=null) {
            val did=currentDid!!; val n=u(a[2])
            results[did]=R(T.NRC,nrc=n,text=nrc(n))
    ...
    private fun sendStatus(g:BluetoothGatt) {
        withCredit(g,"RHMI STATUS"){
            if(statusSent)return@withCredit
            val f=fifo(g)?:return@withCredit
    ...
    private fun next(g:BluetoothGatt) {
        ...
        withCredit(g,"READ ${hd(dids[index])}"){sendRead(g)}
    }
    ...
    private fun timeout(g:BluetoothGatt,did:Int) {
        ...
        handler.postDelayed({
            ...
            waiting=false;currentDid=null;index++;grant(g,"TIMEOUT-RECOVERY")
            handler.postDelayed({next(g)},500)
        },RESPONSE_TIMEOUT_MS)
    }
    ...
    private fun summary() {
        ...
        log("----- ALL POSITIVE DID -----")
        pos.forEach{(d,r)->log("${hd(d)} len=${r.data.size} RAW=${hex(r.data)} | ${r.text}")}
    ...
    private fun mins(b:ByteArray,label:String):String {
        if(b.size<2)return "$label short"
        val m=(u(b[0]) shl 8) or u(b[1])
        return "$label: $m min = ${m/60}:${String.format(Locale.US,"%02d",m%60)}"
    }
