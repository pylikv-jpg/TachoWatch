...
class FullDtcoScannerV3Activity : AppCompatActivity(), FullDtcoScannerV3.Listener {
    ...
    private var scanner: FullDtcoScannerV3? = null
    ...
    private fun buildUi(): View {
        ...
        root.addView(TextView(this).apply {
            text = "DTCO FULL READ-ONLY SCANNER v3"
            textSize = 20f
        })
        ...
        saveButton = Button(this).apply {
            ...
            setOnClickListener {
                ...
                saveLauncher.launch("DTCO_full_scan_v3_$stamp.txt")
            }
        }
...
class FullDtcoScannerV3(
    ...
    companion object {
        ...
        private const val DID_TIMEOUT_MS = 1200L
        private const val STEP_DELAY_MS = 110L
        private const val SUBSCRIBE_DELAY_MS = 180L
        private const val HANDSHAKE_TIMEOUT_MS = 1800L
        private const val RECONNECT_DELAY_MS = 1300L
        private const val MAX_RECONNECTS = 3
        private const val MAX_HANDSHAKE_ATTEMPTS = 2
    ...
    fun start(device: BluetoothDevice, start: Int, end: Int) {
        ...
        log("SAFE MODE: DIAG service; after RHMI open only UDS 0x22")
        log("BLE: serialized FIFO->CREDITS CCCD subscription")
        log("FLOW FIX: DIAG_CREDITS=00 is informational and MUST NOT gate FIFO TX")
        ...
        override fun onDescriptorWrite(g: BluetoothGatt, descriptor: BluetoothGattDescriptor, status: Int) {
            ...
            when {
                ...
                phase == Phase.SUB_CREDITS && uuid == DIAG_CREDITS -> {
                    ...
                    log("HANDSHAKE v3: grant peer RX, then send OPEN regardless of peer CREDITS value")
                    grantPeer(g)
                    handler.postDelayed({ sendOpenAttempt(g) }, 260L)
                }
            }
        }
    ...
    private fun inventory(g: BluetoothGatt) {
        ...
        log("GATT subscribable=$subscribable; v3 activates DIAG FIFO+CREDITS only")
    }
    ...
    private fun incoming(g: BluetoothGatt, c: BluetoothGattCharacteristic, v: ByteArray) {
        ...
        if (c.uuid == DIAG_CREDITS) {
            if (v.isNotEmpty()) {
                ...
                lastPeerCredits = n
                if (n == 0xFF) {
                    scheduleReconnect("peer credits 0xFF")
                    return
                }
                if (n == 0) log("FLOW v3: peer CREDITS=00 observed; continuing by serialized TX policy")
            }
            return
        }
        ...
        if (a.size >= 5 && u(a[0]) == 0x71 && u(a[1]) == 0x03 && u(a[2]) == 0xF2 && u(a[3]) == 0x11) {
            ...
            if (s == 0x10) {
                ...
                scheduleReconnect("RHMI status 0x${h2(s)}")
            }
            return
        }
    ...
    private fun sendOpenAttempt(g: BluetoothGatt) {
        if (!running || phase != Phase.HANDSHAKE || openPositive || gatt != g) return
        if (handshakeAttempts >= MAX_HANDSHAKE_ATTEMPTS) {
            scheduleReconnect("OPEN RHMI no response")
            return
        }
        handshakeAttempts++
        ...
        log("TX OPEN RHMI attempt=$handshakeAttempts peerCredits=$lastPeerCredits started=$ok hex=${hex(packet)}")
        if (!ok) {
            handler.postDelayed({ sendOpenAttempt(g) }, 300L)
            return
        }
        handler.postDelayed({
            if (running && phase == Phase.HANDSHAKE && !openPositive && gatt == g) {
                grantPeer(g)
                handler.postDelayed({ sendOpenAttempt(g) }, 180L)
            }
    ...
    private fun sendStatus(g: BluetoothGatt) {
        if (!running || statusSent || gatt != g) return
        val fifo = g.getService(DIAG_SERVICE)?.getCharacteristic(DIAG_FIFO) ?: return
        val packet = byteArrayOf(1, 1, 0x31, 0x03, 0xF2.toByte(), 0x11)
        val ok = writeNoResponse(g, fifo, packet)
        log("TX RHMI STATUS peerCredits=$lastPeerCredits started=$ok hex=${hex(packet)}")
        if (ok) statusSent = true else handler.postDelayed({ sendStatus(g) }, 250L)
    }
    ...
    private fun sendCurrentDid(g: BluetoothGatt) {
        ...
        grantPeer(g)
        handler.postDelayed({
            if (!running || waitingDid || gatt != g || currentDid > endDid) return@postDelayed
            ...
            val ok = writeNoResponse(g, fifo, packet)
            log("TX DID ${h4(did)} peerCredits=$lastPeerCredits started=$ok")
            if (!ok) {
                handler.postDelayed({ sendCurrentDid(g) }, 220L)
                return@postDelayed
            }
            waitingDid = true
            val token = ++didToken
            ...
            handler.postDelayed({
                if (running && phase == Phase.DID_SCAN && waitingDid && token == didToken && gatt == g) {
                    timeoutCount++
    ...
    private fun grantPeer(g: BluetoothGatt) {
        ...
        log("TX GRANT peer RX +1 started=$ok")
    }
    ...
    @SuppressLint("MissingPermission")
    private fun safeName(device: BluetoothDevice): String =
        try { device.name ?: "DTCO" } catch (_: Throwable) { "DTCO" }
